...
def data_word2vec():
    # 加载词向量模型
    model_word = KeyedVectors.load_word2vec_format(data_dir + 'word2vec\\64\\news_12g_baidubaike_20g_novel_90g_embedding_64.bin', binary = True)
...
def train_lstm(input_dim, embedding_weights, x_train, y_train, x_test, y_test, version):
    if version == None :
        ...
        # 基于之前生成的模型继续训练
        model = load_model(model_dir + version + '\\lstm.h5')
...
# 4、转换为词向量
input_dim,embedding_weights,w2dic = data_word2vec()
# 5、文本转关键词序列号数组
...
# 9、lstm情感训练
version = None
train_lstm(input_dim, embedding_weights, x_train, y_train, x_test, y_test, version)
